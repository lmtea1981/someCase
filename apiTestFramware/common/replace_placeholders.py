        ...
        if '"{' in data and '\\' not in data:
            if '(.+?)' not in data:
                try:
                    ...
                    logger.error(f"{data}:字符串转字典失败！详细：{e}\n%%%===>{data}")

        logger.debug(f"*******正在处理:{type(data)} --  {data}")
...
def convert_string_to_number(value):
    """尝试将字符串转换为 int 或 float，失败则返回原值"""
    if isinstance(value, str):
        try:
            return int(value)  # 先尝试转 int
        except ValueError:
            try:
                return float(value)  # 再尝试转 float
            except ValueError:
                pass
    return value
