...
class SimpleKeywordRetriever(BaseRetriever):
    ...
    def _count_keyword_matches(self, query: str, document: str) -> int:
        ...
        # 去除查询中的标点符号
        query_no_punc = ''.join([c for c in query if c.isalnum() or c.isspace() or '\u4e00' <= c <= '\u9fff'])
        
        # 检测是否为中文查询（包含中文字符）
        is_chinese = any('\u4e00' <= c <= '\u9fff' for c in query_no_punc)
        
        if not is_chinese:
            # 英文查询，按单词匹配
            query_words = query_no_punc.lower().split()
            ...
            self.traces.append(
                TraceEvent(
                    ...
                    data={
                        ...
                        "usage": (
                            response.usage_metadata if response.usage_metadata else None
                        ),
                        "model": "gpt-4o",
                    },
                )
